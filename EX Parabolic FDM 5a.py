...
k = 0.05
n = 10                                              #j-end
ic_f = lambda x : math.sin(2*math.pi*x)            #IC
bc_L = lambda t : 0                                #Left BC
bc_R = lambda t : 0                                #Right BC
#Exact Solution
u = lambda x,t : math.exp(-4*t*math.pi**2)*math.sin(2*math.pi*x)
####################################################################
####################################################################
...
for i in range(m+1) :
    ...
    print("{:.2f}\t {:5.8f}\t {:15.8e}\t {:15.8e}".format(x[i],u(x[i],t_end),w_new[i],error))